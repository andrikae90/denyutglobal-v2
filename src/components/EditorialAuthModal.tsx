...
export const EditorialAuthModal: React.FC<EditorialAuthModalProps> = ({ isOpen, onClose, onAuthenticate, isLoading = false, errorMessage = null }) => {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setLockoutRemainingSeconds((value) => Math.max(0, value - 1)), 1000);
  ...
  useEffect(() => {
    ...
    if (remaining > 0) setLocalError(`Terlalu banyak percobaan gagal. Akses dikunci sementara. Silakan tunggu ${Math.ceil(remaining / 60)} menit lagi.`);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const remaining = checkLockoutStatus();
    if (remaining > 0) { setLockoutRemainingSeconds(remaining); return; }
    const value = passphrase.trim();
    if (!value) { setLocalError('Silakan masukkan kunci akses / passphrase redaksi.'); return; }

    setLocalError(null);
    setIsSubmitting(true);
    try {
      // Verifikasi passphrase hanya di Worker. Tidak ada hash rahasia di bundle browser.
      ...
      if (!response.ok || !data?.token) {
        ...
        if (attempts >= MAX_ATTEMPTS) setLocalError('Terlalu banyak percobaan gagal (5/5). Akses dibekukan sementara selama 5 menit untuk keamanan.');
        else setLocalError(data?.error || `Kunci akses tidak valid. (Sisa percobaan: ${MAX_ATTEMPTS - attempts})`);
        return;
      }
    ...
    <div id="editorial-auth-modal-backdrop" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs" role="dialog" aria-modal="true" aria-labelledby="editorial-auth-title">
      <div id="editorial-auth-modal-container" className="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden text-slate-900">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/80">
          <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-xl bg-rose-100 text-rose-600 flex items-center justify-center"><ShieldCheck className="w-5 h-5" /></div><div><h2 id="editorial-auth-title" className="text-base font-bold">Akses Ruang Redaksi</h2><p className="text-xs text-slate-500">Area terbatas untuk jurnalis & editor DenyutGlobal</p></div></div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="space-y-1.5"><label htmlFor="editorial-passphrase-input" className="block text-xs font-semibold text-slate-700 uppercase tracking-wider">Kunci Akses / Passphrase</label><div className="relative"><div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-slate-400"><Lock className="w-4 h-4" /></div><input id="editorial-passphrase-input" type={showPassword ? 'text' : 'password'} value={passphrase} onChange={(event) => { setPassphrase(event.target.value); if (!isLocked) setLocalError(null); }} disabled={isBusy || isLocked} placeholder={isLocked ? 'Form dibekukan...' : 'Masukkan kata sandi redaksi...'} autoFocus={!isLocked} autoComplete="current-password" className="w-full pl-10 pr-11 py-2.5 bg-slate-50 border border-slate-300 focus:border-rose-500 focus:bg-white rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-hidden focus:ring-2 focus:ring-rose-500/20 disabled:bg-slate-100 disabled:text-slate-400" /><button type="button" onClick={() => setShowPassword((value) => !value)} disabled={isBusy || isLocked} aria-label={showPassword ? 'Sembunyikan kata sandi' : 'Tampilkan kata sandi'} className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 hover:text-slate-600 disabled:opacity-50">{showPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button></div></div>
          ...
          <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-slate-100"><button type="button" onClick={onClose} disabled={isBusy} className="px-4 py-2 text-xs sm:text-sm font-medium text-slate-700 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 rounded-xl disabled:opacity-50">Batal</button><button type="submit" disabled={isBusy || isLocked || !passphrase.trim()} className="px-5 py-2 text-xs sm:text-sm font-semibold text-white bg-rose-600 hover:bg-rose-700 rounded-xl shadow-xs flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">{isBusy ? <><Loader2 className="w-4 h-4 animate-spin" /><span>Memverifikasi...</span></> : <span>Masuk</span>}</button></div>
        </form>
      </div>
    </div>
  );
